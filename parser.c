...
const char *token_str(token_t token) {
    static_assert(TOKEN_NUM_TOKENS == 72);
    switch (token) {
        ...
        case TOKEN_CONTROL: return "TOKEN_CONTROL";
...
token_span parse_token(string_stream *in) {
    ...
    while (true) {
        ...
        if (iscntrl(peek.c) || peek.c < 0) {
            return consume_token(in, TOKEN_CONTROL, 1);
        }
...
int main() {
    ...
    while (true) {
        ...
        printf("\"" sv_fmt "\" (%02x) => %s\n", sv_args(token.span), token.span.str ? *(uint8_t*)token.span.str : 0, token_str(token.type));
